...
def LPFilterKaiser(signal, cutoff=100, transitionWidth=40, rippleDB=20):
    '''
    Filters the input signal using a Kaiser window filter
    :param signal: neo.analogsignal, input signal to be filtered
    :param cutoff: float, cutoff frequency for the kaiser filter
    :param transitionWidth: float, transition width for the kaiser filter
    :param rippleDB: float, rippleDB of the kaiser filter
    :return: delay: float, amount of delay caused by the filter in units of the signal dimension
             filteredSignal: neo.analogsignal, filtered signal
    '''
...
def shouldBeIgnored(resp, intervalToIgnore):
    '''
    Returns whether the signal resp has any intersection with intervalToIgnore
    :param resp: neo.AnalogSignal
    :param intervalToIgnore: 2 member float iterable
    :return: bool
    '''
...
for expName in expNames:
    ...
    for resp in resps[265.0]:
        ...
        for typeInd, tpye in enumerate(types):
            ...
            temp = resp[tpye]
            if shouldBeIgnored(temp, intervalToIgnore):
                break
    ...
    ax0.grid(True)
    ax0.set_xticks(np.arange(-1, 2, 0.1))
    fig0.canvas.draw()
    ...
    if not xs_accepted:
        ...
        ax1[0, 0].set_title('None of the solutions are acceptable. Plotting the best among the solutions')
