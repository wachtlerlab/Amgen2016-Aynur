...
class RawDataProcessor():
    ...
    def downSampleVibSignal(self):
        ...
        self.vibrationSignalDown = AnalogSignal(signal=vibSigDownMag,
    ...
    #*******************************************************************************************************************
    def __init__(self, expName, dirpath, readOnly=False):
    ...
    def getStimulusEpochs(self, minDur=9 * qu.ms, minGap=14 * qu.ms, vibThresholdSig=1 * qu.um):
        """
        Using direct thesholding of signal amplitude and adaptive threshold to determine a valid gap between stimuli.
        :return:
        """

        thresholdedSig = np.asarray(np.abs(self.vibrationSignalDown) > vibThresholdSig, int)
    ...
    def sortSegments(self):
        ...
        nearest = lambda a, a0: a[np.abs(a-a0).argmin()]
        ...
        for stimInd in range(len(self.stimDurs)):
            ...
            if (self.stimDurs[stimInd] < 0.5 * qu.s) and (self.hasPulses):
                ...
                nearestPulseDur = nearest(writtenPulseDurs, self.stimDurs[stimInd])
                nearestPulseInterval = nearest(writtenPulseIntervals, self.forwardISIs[stimInd])
                ...
                if (self.backwardISIs[stimInd] > 0.5 * qu.s) and (self.forwardISIs[stimInd] < 0.5 * qu.s):
                    ...
                    if diff > 6 * qu.ms:
                            print('Large Difference of ' + str(diff) + ' between estimated and ' \
                                        'written values of Pulse Interval at stimInd ' + str(stimInd) + str(stimTime))
                ...
                if (self.backwardISIs[stimInd] < 0.5 * qu.s) and (self.forwardISIs[stimInd] > 0.5 * qu.s):
                        ...
                        nearestPulseInterval = nearest(writtenPulseIntervals, self.backwardISIs[stimInd])
                        key = (simpleFloat(nearestPulseDur), simpleFloat(nearestPulseInterval))
            ...
            # if it's a continuous stimulus
                ...
                nearestFreq = simpleFloat(nearest(writtenFreqs, self.stimFreqs[stimInd]))
                freqDiff = (nearestFreq * qu.Hz) - self.stimFreqs[stimInd]
                if freqDiff > 1 * qu.Hz:
                    print('Large difference  of ' + str(freqDiff) + ' between the estimated and written values '
                                                            'of frequency at stimInd=' + str(stimInd))
                    print('This response will be ignored')
                else:
                    self.contInput.setdefault(nearestFreq, []).append(stimInd)

        # if self.hasPulses:
        #     #here, pulse number is defined as min(number of pulses per train) for all trains with the same (duration, interval)
        #     self.pulseNumbers = {}
        #
        #     for pulseKey, stimIndsSet in self.pulseInput.iteritems():
        #
        #         self.pulseNumbers[pulseKey] = min([len(stimInds) for stimInds in stimIndsSet])
    ...
    def addStimAndSpikes(self, tagCount, type, start, extent, blk, refs, sec):
        ...
        if len(spikes):
            positions = createPosDA('Spikes' + str(tagCount + 1), simpleFloat(spikes), blk)
