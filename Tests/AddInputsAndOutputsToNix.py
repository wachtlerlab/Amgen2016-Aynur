...
for ename in mio.GetAvaliableIds():
    ...
    for l in [n for n in names if "Trial" == n[:5]]:
        sig = [k for k in seg.analogsignals if k.name==l]
        spk = [k for k in seg.spiketrains if k.name==l]
        if len(sig)>0 and len(spk)>0: f.add_exp_output(sig[0], spk[0], name=sig[0].name+"-DuringStimulus")
    l = "PredictedInput1"
    ...
    for l in [n for n in names if "Trial" == n[:5]]:
        ...
        if len(sig)>0 and len(spk)>0: f.add_exp_output(sig[0], spk[0], name=sig[0].name+"-AfterStimulus")
